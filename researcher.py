...
TAPI_HEADERS = {"Authorization": f"Bearer {TRANSCRIPTAPI_KEY}"}
TAPI_BASE = "https://transcriptapi.com/api/v2"
...
SEED_QUERIES = [
    "apache iceberg python pyiceberg implementation",
    "claude MCP server tool calling python implementation",
    "duckdb python lakehouse medallion pipeline",
    "pyspark delta lake merge upsert python",
...
def search_youtube(query):
    ...
    try:
        r = requests.get(
            f"{TAPI_BASE}/youtube/search",
            headers=TAPI_HEADERS,
            params={"q": query, "type": "video", "limit": 10},
            timeout=30
        )
        print(f"📡 TranscriptAPI search | Status: {r.status_code}")
        ...
        print(f"⚠️ Search error: {r.text[:200]}")
        return []
...
def get_transcript(video_id):
    ...
    try:
        r = requests.get(
            f"{TAPI_BASE}/youtube/transcript",
            headers=TAPI_HEADERS,
            params={
                "video_url": video_id,
                "format": "text",
        ...
        print(f"📡 TranscriptAPI transcript | Status: {r.status_code}")
        if r.status_code == 200:
            return r.json().get("transcript", "")
        if r.status_code == 402:
            print("💳 Out of credits — visit https://transcriptapi.com/billing")
        else:
            print(f"⚠️ Transcript error: {r.text[:200]}")
        return None
    except Exception as e:
...
def get_existing_skills():
    ...
    return [p.stem for p in SKILLS_DIR.glob("*.py") if not p.stem.startswith("test_")]
...
def save_skill_to_registry(skill_data, video):
    ...
    readme_path = SKILLS_DIR / f"{slug}_readme.md"
    ...
    explain_prompt = (
        f"You are writing documentation for a data engineering skill extracted from a YouTube tutorial.\n\n"
        f"Skill name: {skill_data['skill_name']}\n"
        f"Source video: {video['title']}\n\n"
        f"Code:\n{skill_data['implementation_code']}\n\n"
        "Write a concise README.md with these sections:\n"
        "## What this is\n"
        "## The problem it solves\n"
        "## How to use it (with a short code example)\n"
        "## What real-world tool this relates to\n"
        "## Limitations\n\n"
        "Keep it practical. Target audience: data engineers who want to understand "
        "the concept quickly and know when to apply it."
    )
    readme = client.models.generate_content(model=SYNTHESIS_MODEL, contents=explain_prompt)
    with open(readme_path, "w", encoding="utf-8") as f:
        f.write(f"# {skill_data['skill_name']}\n\n")
        f.write(f"**Source:** [{video['title']}](https://youtube.com/watch?v={video['id']})\n")
        f.write(f"**Added:** {datetime.now().strftime('%Y-%m-%d')}\n\n")
        f.write(readme.text.strip())

    print(f"📚 Saved to registry: skills/{slug}.py + {slug}_readme.md")
...
def scout_and_synthesize():
    ...
    technical_markers = ["import", "def ", "async", "class ", "pip", "install"]
    ...
    for video in videos:
        ...
        if not any(marker in transcript.lower() for marker in technical_markers):
            print("⏭ No technical content detected — skipping.")
            continue

        print(f"✅ Technical content found — synthesising skill...")

        prompt = (
            f"Goal: {GOAL}\n\n"
            f"Extract the exact technical implementation from this transcript:\n"
            f"{transcript[:12000]}\n\n"
            "Ignore all marketing, intros, and opinions. Focus purely on the code logic.\n"
            "Output ONLY a raw JSON object (no markdown fences) with exactly these keys:\n"
            ...
            "3. 'unit_test': pytest test functions. Do not import from proposed_skill — "
            "that import is added automatically."
        )
        ...
        for iteration in range(1, MAX_REPAIR_ITERATIONS + 1):
            ...
            if passed:
                ...
                print(f"💾 Next query saved.")
                return

            print(f"🔁 Iteration {iteration} failed. Feeding error back to Gemini...")
            print(f"--- TEST OUTPUT ---\n{test_output[:400]}\n---")

            repair_prompt = (
                f"This Python skill failed its pytest tests.\n\n"
                f"=== implementation_code ===\n{skill_data['implementation_code']}\n\n"
                f"=== unit_test ===\n{skill_data['unit_test']}\n\n"
                f"=== test output (loss signal) ===\n{test_output}\n\n"
        ...
        print(f"⚠️ Max iterations reached for '{skill_data['skill_name']}' — skipping.")

    print("⚠️ No verified skill found in this batch.")
